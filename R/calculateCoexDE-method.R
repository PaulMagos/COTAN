...
#' pcaGeneCoex
#'
#' Calculate the PCA of geneCoex.
#'
#' @param geneCoex the coex `matrix` on the genes
#' @param GDI the GDI `data.frame`
#' @param varThreshold genes with GDI less than `varThreshold` are excluded from
#'   `geneCoex` columns
#' @param idThreshold genes with GDI less than `idThreshold` are excluded from
#'   `geneCoex` rows
#' @param actCoexSpace Boolean, when `TRUE` the Coex space is applied
#'
#' @returns a list with the PCA and the variance explained for each component
#'
#' @export
pcaGeneCoex <- function(geneCoex,
                        GDI,
                        varThreshold = 1.5,
                        idThreshold = 1.25,
                        pcaDim = 4,
  ...
  geneCoex <- geneCoex[idGenes, varGenes]
  cat("Gene coex dimensions: ", dim(geneCoex), "\n")
  cat("GDI mean of genes on columns (vars):", mean(GDI[varGenes]), "\n")
  cat("GDI mean of genes on rows (id):", mean(GDI[idGenes]), "\n")

  cat("coexpresion space:")
  if(actCoexSpace){
    cat(" yes\n")
    if(numCells <= 0){
      stop("The number of cells must be greater than 0 if actCoexSpace is TRUE")
    }
    #pcaStruct$x <- tanh(pcaStruct$x * sqrt(numCells))
    geneCoex <- tanh(geneCoex * sqrt(numCells))
  } else{
    cat(" no\n")
  }

  cat("PCA starts \n")
  pcaStruct <- irlba::prcomp_irlba(geneCoex, n = pcaDim, scale. = F)
  rownames(pcaStruct$x) <- idGenes
